...
void Foam::janafTThermo<EquationOfState>::checkInputData() const
{
    if (Tlow_ >= Thigh_)
    {
...
template<class EquationOfState>
Foam::janafTThermo<EquationOfState>::janafTThermo(const dictionary& dict)
:
...
template<class EquationOfState>
void Foam::janafTThermo<EquationOfState>::write(Ostream& os) const
{
...
(
    Ostream& os,
    const janafTThermo<EquationOfState>& jt
)
{
    jt.write(os);
    return os;
