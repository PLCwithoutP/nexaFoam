...
Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::StandardNeChemistryModel
(
    ...
    dcdt_(nSpecie_)
{
    ...
    Info<< "StandardNeChemistryModel: Number of species = " << nSpecie_
...
) const
{
    ...
    forAll(reactions_, i)
    {
        const neReaction<ThermoType>& R = reactions_[i];

        scalar omegai = omega
        (
            R, c, TTR, p, pf, cf, lRef, pr, cr, rRef
        );
...
template<class ReactionThermo, class ThermoType>
Foam::scalar Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::omegaI
(
    const label index,
    const scalarField& c,
    const scalar TTR,
...
{
    const neReaction<ThermoType>& R = reactions_[index];
    scalar w = omega(R, c, TTR, p, pf, cf, lRef, pr, cr, rRef);
    return(w);
}


template<class ReactionThermo, class ThermoType>
Foam::scalar Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::omega
(
...
template<class ReactionThermo, class ThermoType>
void Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::derivatives
(
    const scalar time,
    const scalarField& c,
    scalarField& dcdt
) const
{
    ...
    for (label i=0; i<nSpecie_; i++)
    {
        cp += c_[i]*specieThermo_[i].cpTR(p, TTR);
    }
    cp /= rho;
    ...
    for (label i = 0; i < nSpecie_; i++)
    {
        const scalar hi = specieThermo_[i].ha(p, TTR);
        dT += hi*dcdt[i];
    }
    dT /= rho*cp;
...
template<class ReactionThermo, class ThermoType>
void Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::jacobian
(
    const scalar t,
    const scalarField& c,
    scalarField& dcdt,
...
Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::tc() const
{
...
template<class ReactionThermo, class ThermoType>
Foam::tmp<Foam::volScalarField>
Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::Qdot() const
{
...
Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::calculateRR
(
    const label ri,
    const label si
) const
...
template<class ReactionThermo, class ThermoType>
void Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::calculate()
{
    if (!this->chemistry_)
    {
        return;
...
Foam::scalar Foam::StandardNeChemistryModel<ReactionThermo, ThermoType>::solve
(
